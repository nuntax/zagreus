        ...
        if (ImGui::Button("DispatchBeginPlay")) {
            FunctionQueue::AddFunction([]() {
                ...
                for (int i = 1; i < AllPCs.Num(); i++) {
                    ...
                    console.AddLog("Spawning for PlayerController: %s", PC->GetFullName().c_str());
                    if (i >= 2) {
                        PC->TeamComponent->bIsRedTeam = false;
                    } else {
                        PC->TeamComponent->bIsRedTeam = true;
                    }
                    console.AddLog("1Player's Team: %s", PC->TeamComponent->bIsRedTeam ? "Red" : "Blue");
                }
                ...
                for (int i = 1; i < AllPCs.Num(); i++) {
                    ...
                    auto PC = static_cast<SDK::ABP_AGPlayerController_C*>(AllPCs[i]);
                    console.AddLog("Spawning for PlayerController: %s", PC->GetFullName().c_str());
                    console.AddLog("Player's Team: %s", PC->TeamComponent->bIsRedTeam ? "Red" : "Blue");
