...
void StartMatch() {
  ...
  for (int i = 1; i < AllPCs.Num(); i++) {
    ...
    std::cout << "Spawning for PlayerController: " << PC->GetFullName()
              << std::endl;
    int j = 2;
    if (MaxPlayers == 5) {
      j = 3;
...
void InitializeServer() {
  ...
  std::wstring servercommand =
      map_wstring + L"?listen?game=AimGods.BP_AGGameMode_C";
  MaxPlayers = j["maxplayers"].template get<int>();

  std::cout << "MaxPlayers: " << MaxPlayers << std::endl;
  std::cout << "Map: " << map << std::endl;
  SDK::UGameplayStatics::GetDefaultObj()->SetEnableWorldRendering(GWorld,
                                                                  false);
...
void hProcessEvent(SDK::UObject *Object, SDK::UFunction *Function,
  ...
  if (GetAsyncKeyState(VK_CAPITAL) & (1 << 16) &&
          ShouldLog(Function->GetFullName()) ||
      (Hooks::LogProcessEvent && ShouldLog(Function->GetFullName()))) {
...
void Hooks::HookFunctions() {
  rProcessEvent =
      (tProcessEvent)Mem::Scan(GetModuleHandleW(nullptr),
                               ...
                               "xxxxxxxxxxxxxxxx????");
  rNotifyBeginplay = (tNotifyBeginplay)Mem::Scan(
      GetModuleHandleW(nullptr),
      reinterpret_cast<const unsigned char *>(
          "\x40\x55\x53\x48\x8D\x6C\x24\x00\x48\x81\xEC\x00\x00\x00\x00\x48\x8B"
          "\x05\x00\x00\x00\x00\x48\x33\xC4\x48\x89\x45\x2F"),
      "xxxxxxx?xxx????xxx????xxxxxxx");
  rHandleHasMatchStarted = (tHandleHasMatchStarted)Mem::Scan(
      GetModuleHandleW(nullptr),
      reinterpret_cast<const unsigned char *>(
          "\x4C\x8B\xDC\x55\x41\x56\x49\x8D\x6B\xA1\x48\x81\xEC\x00\x00\x00\x00"
          "\x48\x8B\x05\x00\x00\x00\x00\x48\x33\xC4\x48\x89\x45\x37\x49\x89\x5B"
          "\x10"),
      "xxxxxxxxxxxxx????xxx????xxxxxxxxxxx");
  rRespawnPlayer = (tRespawnPlayer)Mem::Scan(
      GetModuleHandleW(nullptr),
      reinterpret_cast<const unsigned char *>(
          "\x48\x85\xD2\x0F\x84\x00\x00\x00\x00\x56\x41\x56\x48\x83\xEC\x78"),
      "xxxxx????xxxxxxx");
